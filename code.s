...
#537395210 0x2008000a  [executa:] 	addi $8, $0, 10       			; 04: addi $t0, $zero, 10 	 #04-01 nr maximo de iteracoes (10)
#537460737 0x20090001  				addi $9, $0, 1                  ; 08: addi $t1, $zero, 1  	 #08-02 aux para incremento (1)
#537526274 0x200a0002  				addi $10, $0, 2                 ; 12: addi $t2, $zero, 2  	 #12-03 aux para nr 2
#277348355 0x10880003  [loop_exec1:]beq $4, $8, 16 (sai_loop-20)/4  ; 16: beq $a0, $t0, sai_loop #16-04 enquanto $a0 < $t0 => farah 10 iteracoes   (desloca 3 palavras à frente)
#8986656   0x00892020  				add $4, $4, $9                  ; 20: add $a0, $a0, $t1   	 #20-05 $a1 ++   no fim do loop $a0 terah 10
#13185058  0x00c93022  				sub $6, $6, $9                  ; 24: sub $a2, $a2, $t1 	 #24-06 $a2 --   no fim do loop $a2 terah 2
#134217732 0x08000004  				j 0x00000004 (loop_exec1/4)     ; 28: j loop_exec1 		 	 #28-07 volta para inicio do loop (desloca 04 palavras para trás -04)
#8929322   0x0088402a  [sai_loop:]	slt $8, $4, $8       			; 32: slt $t0, $a0, $t0 	 #32-08 setando $t0 para zero  (como $a0==10 e $t0==10, entao $t0 = 0)
#549912577 0x20c70001  				addi $7, $6, 1                  ; 36: addi $a3, $a2, 1		 #36-09 $a3 = 3
#14702629  0x00e05825  				or $11, $7, $0                  ; 40: or 	$t3, $a3, $zero  #40-10 $t3 = 3
#552337409 0x20ec0001  				addi $12, $7, 1                 ; 44: addi $t4, $a3, 1		 #44-11 $t4 = 4
#25518116  0x01856024  				and $12, $12, $5                ; 48: and  $t4, $t4, $a1  	 #48-12 $t4 = 4
#361496579 0x158c0003  				bne $12, $12, 16 (naopula-56)/4 ; 52: bne $t4, $t4, naopula	 #52-13 nao efetiva o bne, pois $t4 == $t4  :-)    (desloca 3 palavras à frente)
#538443876 0x20180064  				addi $24, $0, 100               ; 56: addi $t8, $zero, 100	 #56-14 carrega endereco para salva na memoria
#2936668160 0xaf0a0000  			sw $10, 0($24)                  ; 60: sw $t2, 0($t8) 		 #60-15 salva $t2 (tem vlr 2) na memoria
#2400321536 0x8f120000  			lw $18, 0($24)                  ; 64: lw $s2, 0($t8) 	  	 #64-16 recupera vlr 2 para s2
#1407188992 0x53E00000 [naopula:]	jr $31                			; 68: jr $ra    			 #68-17 fim de executa. Volta para a main.   0101 0011 1110 0000 0000 0000 0000 0000 (0x53E00000) 
#813957120 0x30840000  [main:]		andi $4, $4, 0           		; 72: andi $a0, $a0, 0		 #72-18 $a0 tem todos os bits como 0
#537264127 0x2005ffff  				addi $5, $0, -1                 ; 76: addi $a1, $zero, -1    #76-19 $a1 tem todos os bits como 1
#816185356 0x30a6000c  				andi $6, $5, 12                 ; 80: andi $a2, $a1, 12	 	 #80-20 $a2 tem 12 para ser decrementado em executa
#538443777 0x20180001  				addi $24, $0, 1                 ; 84: addi $t8, $zero, 1	 #84-21 recupera endereco de executa (#01)
#1461649408 0x571f0000  			jalr $24, $31                   ; 88: jalr $t8, $ra, $t8 	 #88-22 desvia fluxo para funcao executa  0101 0111 0001 1111 0000 0000 0000 0000 (0x571f0000)
