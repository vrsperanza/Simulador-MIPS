...
/*
Leonardo Augusto Franco do Amaral - 8508911 - MEF, banco de registradores e multiplexadores
Miguel de Mattos Gardini - 10295728 - Robustez do programa, Lógica da ULA, da unidade de controle e da ALU control
Vitor Rossi Speranza - 10262523 - Conexão entre os componentes, threads e lógica dos componentes restantes
Ygor Franco de Camargo Pontelo - 10295631 - Robustez do programa, Lógica da ULA, da unidade de controle e da ALU control
*/
...
REGISTERS * REGISTERS_init(int * readRegister1, int * readRegister2, int * writeRegister,
	...
	registers->registers = (int*)calloc(sizeof(int), 32);
...
MEMORY * MEMORY_init(char * memRead, char * memWrite, int * address, int * writeData,
	...
	memory->memory = (unsigned char*)calloc(sizeof(unsigned char), byteCapacity);
...
void* CONTROL_main(void * args){
	...
	while(1){
		...
		*(signals->output) |= ((s==8) | (s==12) | (s==10)) << 6;	//ALUOp0
...
CONTROL * CONTROL_init(char * option, int * output, sem_t * dependency){
...
int main()
{
	...
	CONTROL * UC = CONTROL_init(&IROut_26_31, &UCOut, IRSplit_26_31->done);
	...
	ALU_CONTROL * ALUControl = ALU_CONTROL_init(&IROut_0_5, &ALUControlOut, &UCOut_ALUop, IRSplit_0_5->done, UCOutSplit_ALUop->done);
